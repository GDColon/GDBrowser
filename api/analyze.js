...
module.exports = async (app, req, res, level) => {
  ...
  function sortObj(obj, sortBy) {
    ...
    var keys = !sortBy
      ...
      : Object.keys(obj).sort((a, b) => obj[b][sortBy] - obj[a][sortBy]);
    keys.forEach(x => {
      sorted[x] = obj[x];
    });
    return sorted;
  ...
  orbArray = data
    .filter(x => x.orb)
    .reduce((a, b) => {
      //stolen from https://stackoverflow.com/questions/45064107/how-do-i-group-duplicate-objects-in-an-array
  ...
  triggerArray = data
    ...
    .reduce((a, b) => {
      ...
      return (
        i === -1 ? a.push({ trigger: b.trigger, count: 1 }) : a[i].count++, a
      );
    }, [])
    .sort(function(a, b) {
      return parseInt(b.count) - parseInt(a.count);
  ...
  Object.keys(data[0]).forEach(x => {
    ...
    if (val[1] == "list") val = init[val[0] + "s"][property];
      ...
      colorList.forEach((x, y) => {
        ...
        keys.forEach(k => {
          if (colorStuff.properties[k])
            colorObj[colorStuff.properties[k]] = color[k];
        });
        ...
        let specialColors = colorList
          ...
          .sort(function(a, b) {
            return (
              specialSort.indexOf(a.channel) > specialSort.indexOf(b.channel)
            );
          });
        let regularColors = colorList
          .filter(x => !isNaN(x.channel))
          .sort(function(a, b) {
